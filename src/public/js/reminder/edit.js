Vue.component('edit-component', {
    template: `
        <div class="bg-white lg:w-6/12 md:7/12 w-8/12 shadow-3xl rounded-xl">
            ...
            <form class="p-12 md:p-24">
                ...
                <button type="button" class="bg-gradient-to-b from-gray-700 to-gray-900 font-medium p-2 md:p-4 text-white uppercase w-full rounded" @click="edit">Edit</button>
            </form>
        </div>
    `,
    props: ['reminderdata', 'reminders'],
    data() {
        return {
            id: '',
            reminder: {
    ...
    methods: {
        edit: async function () {
            ...
            await axios.put('/api/reminders/'+_this.id, _this.reminder)
                .then(function (response) {
                    const index = _this.reminders.findIndex(reminder => reminder.id === _this.id)
                    if (index != -1) {
                        _this.reminders[index].id = response.data.data.id
                        _this.reminders[index].title = response.data.data.title
                        _this.reminders[index].description = response.data.data.description
                        _this.reminders[index].remind_at = response.data.data.remind_at
                        _this.reminders[index].event_at = response.data.data.event_at
                    }
